/**
 * ISO-3166 country list with ITU E.164 dial codes.
 *
 * `iso2` is the two-letter country code (uppercase). The flag emoji is
 * derived from `iso2` at render time by mapping each ASCII letter to its
 * regional-indicator code point, so we don't ship any image assets.
 */

export interface Country {
  /** Two-letter uppercase ISO-3166 code, e.g. "NG". */
  iso2: string;
  /** Human-readable name. */
  name: string;
  /** Leading "+" international dial code, e.g. "+234". */
  dialCode: string;
}
...
export const COUNTRIES: Country[] = [
  ...
  { iso2: "VG", name: "British Virgin Islands", dialCode: "+1284" },
  ...
  { iso2: "MP", name: "Northern Mariana Islands", dialCode: "+1670" },
  ...
  { iso2: "BL", name: "Saint Barthélemy", dialCode: "+590" },
...
export function flagEmoji(iso2: string): string {
  ...
  return (
    String.fromCodePoint(A + code.charCodeAt(0)) +
    String.fromCodePoint(A + code.charCodeAt(1))
  );
}
...
/**
 * Best-effort parse of an E.164-ish phone string into country + national
 * portion. Returns `null` if nothing matches. Tries longer dial codes first
 * so "+1868" (Trinidad) wins over "+1" (US) on the same input.
 */
export function parsePhone(
  ...
  for (const country of sorted) {
    if (clean.startsWith(country.dialCode)) {
      return { country, national: clean.slice(country.dialCode.length) };
    }
  }
  return null;
}
