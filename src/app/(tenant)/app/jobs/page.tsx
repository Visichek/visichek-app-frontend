...
export default function JobsPage() {
  ...
  const columns: ColumnDef<JobRecord>[] = [
    ...
    },
    {
      ...
      header: "",
      cell: ({ row }) => {
        const href = `/app/jobs/${row.original.taskId}`;
    ...
    return (
      <div className="space-y-4">
        <PageHeader
          ...
          description="Background writes you've triggered in this tenant, newest first."
        />
        <ErrorState
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Recent activity"
        description="Background writes you've triggered in this tenant. Failed jobs are also delivered to your notifications bell."
        actions={
          <Tooltip>
            <TooltipTrigger asChild>
              <div>
      ...
      <DataTable
        ...
        mobileCard={(job) => (
          <JobMobileCard
            job={job}
            isLoadingLink={loadingHref === `/app/jobs/${job.taskId}`}
            onOpen={() => handleNavClick(`/app/jobs/${job.taskId}`)}
          />
        )}
      />
    </div>
