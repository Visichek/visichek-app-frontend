      ...
      {prefsSection && (
        <>
          ...
          <section>
            ...
            <div className="space-y-1">
              <SettingsSelect id="language" label="Language" description="Display language for the interface" value={userSettings?.language ?? "en"} onValueChange={(v) => handleUpdate({ language: v })} options={[{ value: "en", label: "English" }, { value: "fr", label: "French" }, { value: "es", label: "Spanish" }]} />
              ...
              <SettingsSelect id="dateFormat" label="Date format" description="How dates appear across the app" value={userSettings?.dateFormat ?? "DD/MM/YYYY"} onValueChange={(v) => handleUpdate({ dateFormat: v as DateFormat })} options={[{ value: "DD/MM/YYYY", label: "DD/MM/YYYY" }, { value: "MM/DD/YYYY", label: "MM/DD/YYYY" }, { value: "YYYY-MM-DD", label: "YYYY-MM-DD" }]} />
              <SettingsSelect id="timeFormat" label="Time format" description="12-hour or 24-hour clock" value={userSettings?.timeFormat ?? "24h"} onValueChange={(v) => handleUpdate({ timeFormat: v as TimeFormat })} options={[{ value: "12h", label: "12-hour" }, { value: "24h", label: "24-hour" }]} />
            </div>
          </section>
        </>
      )}
