      ...
      {notifsSection && (
        <>
          ...
          <section>
            ...
            <div className="space-y-1">
              <SettingsToggle id="emailOnVisitorCheckIn" label="Visitor check-in" description="Get notified when a visitor checks in at your location" checked={notifPrefs?.emailOnVisitorCheckIn ?? false} onCheckedChange={(v) => handleNotifUpdate({ emailOnVisitorCheckIn: v })} />
              <SettingsToggle id="emailOnAppointmentReminder" label="Appointment reminders" description="Receive reminders before upcoming appointments" checked={notifPrefs?.emailOnAppointmentReminder ?? true} onCheckedChange={(v) => handleNotifUpdate({ emailOnAppointmentReminder: v })} />
              <SettingsToggle id="emailOnIncident" label="Incident alerts" description="Be notified when a new security incident is created" checked={notifPrefs?.emailOnIncident ?? true} onCheckedChange={(v) => handleNotifUpdate({ emailOnIncident: v })} />
