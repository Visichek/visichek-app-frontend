...
export default async function VisitorsPendingPage() {
  const session = await getServerTenantSession();
  const qc = createServerQueryClient();

  if (session) {
    const params = { state: "pending_approval" as const };
    // Key matches `useTenantCheckins(tenantId, { state: 'pending_approval' })`
    // — see src/features/checkins/lib/query-keys.ts checkinKeys.list.
    await ssrPrefetch(
      qc,
  ...
  return (
    <HydrationBoundary state={dehydrateState(qc)}>
      <VisitorsPageClient activeState="pending_approval" />
    </HydrationBoundary>
  );
}
